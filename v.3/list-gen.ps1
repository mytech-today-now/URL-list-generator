...
.DESCRIPTION
    Extracts file URLs from web directory listings and generates text/CSV/JSON files
...
.EXAMPLE
    list-gen https://example.com/files/ -OutputFormat JSON -OutputFile results.json
    # Export results as JSON

.EXAMPLE
    list-gen -InputFile urls.txt -OutputFormat CSV -OutputFile report.csv
...
#   Suppress with: # [PSScriptAnalyzer: SuppressMessage('PSUseShouldProcessForStateChangingFunctions', 'Justification=Function implements per-item ShouldProcess calls and confirmation logic')]
# PSUseDeclaredVarsMoreThanAssignments: Some variables are intentionally assigned
#   for documentation/debugging purposes.
#   Suppress with: # [PSScriptAnalyzer: SuppressMessage('PSUseDeclaredVarsMoreThanAssignments', 'Justification=Variables assigned for documentation and future use')]

# [PSScriptAnalyzer: SuppressMessage('PSUseShouldProcessForStateChangingFunctions', 'Invoke-ListGenCLI', 'Justification=Function implements per-item ShouldProcess calls and confirmation logic')]
# [PSScriptAnalyzer: SuppressMessage('PSUseDeclaredVarsMoreThanAssignments', 'Invoke-ListGenCLI', 'Justification=Variables assigned for documentation and future use')]
...
.PARAMETER OutputFormat
    Output format for result summary. Default: Text.
    Valid values: Text, JSON, CSV

.PARAMETER OutputFile
    Optional path for result summary file. If not specified, results are displayed only.
...
function Invoke-ListGenCLI {
    ...
    if ($OutputFile) {
        try {
            ...
            switch ($OutputFormat) {
                'JSON' {
                    $exportResults | ConvertTo-Json -Depth 5 | Set-Content -Path $OutputFile -Encoding UTF8NoBOM
                    Write-Host "Results exported to JSON: $OutputFile" -ForegroundColor Green
                }
                'CSV' {
                    $exportResults | Export-Csv -Path $OutputFile -NoTypeInformation -Encoding UTF8NoBOM
                    Write-Host "Results exported to CSV: $OutputFile" -ForegroundColor Green
                }
                default {
                    $exportResults | Format-Table -AutoSize | Out-String -Width 4096 | Set-Content -Path $OutputFile -Encoding UTF8NoBOM
                    Write-Host "Results exported to Text: $OutputFile" -ForegroundColor Green
                }
            }
            Write-InfoLog -Message "Exported results" -Context @{ format = $OutputFormat; path = $OutputFile }
        }
        catch {
            Write-ErrorLog -Message "Failed to export results" -Context @{ format = $OutputFormat; path = $OutputFile; error = $_.Exception.Message } -Exception $_.Exception
